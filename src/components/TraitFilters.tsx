...
export default function TraitFilters({ traits, onFilterChange }: TraitFiltersProps) {
  ...
  return (
    <div className="bg-gradient-to-b from-purple-900/40 to-purple-800/20 rounded-xl border border-purple-500/30 p-4 backdrop-blur-sm sticky top-4 max-h-[calc(100vh-2rem)]">
      ...
      <div className="space-y-3 overflow-y-auto pr-2" style={{ maxHeight: 'calc(100vh - 8rem)' }}>
        {Object.entries(traits).map(([traitType, values]) => (
          <div key={traitType} className="border-b border-purple-500/20 pb-3">
            <button
              onClick={() => toggleTraitType(traitType)}
              className="w-full flex items-center justify-between text-left text-white hover:text-purple-300 transition-colors"
            >
                    ...
                    {values.map(value => (
                      <div key={`${traitType}-${value}`} className="flex items-center">
                        <input
                          ...
                          className="w-4 h-4 rounded border-purple-500 text-purple-600 focus:ring-purple-500 focus:ring-offset-purple-900"
                        />
                        <label
                          htmlFor={`${traitType}-${value}`}
                          className="ml-2 text-sm text-purple-200 hover:text-white cursor-pointer truncate"
                        >
                          {value}
                        </label>
                      </div>
        ...
        {Object.keys(traits).length === 0 && (
          <p className="text-purple-400 text-sm italic">No traits available</p>
        )}
      </div>
    </div>
  );
